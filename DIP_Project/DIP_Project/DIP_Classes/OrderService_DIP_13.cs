...
{
	public class OrderService_DIP_13
	{
        public List<int> OrderList = new List<int>();
        public int OrderPrice { get; set; }
        IPrice_DIP_13 OrderPriceService;
        User_DIP_13 User;
        Products_DIP_13 Products;
		CargoService_DIP_13 Cargo;


		public OrderService_DIP_13(User_DIP_13 User)
		{
			this.User = User;
			Products = new Products_DIP_13();
		}
		...
		public void CreateOrder(int CardPassword)
		{
			...
			Cargo = new CargoService_DIP_13(User.CustomerNumber + User.CustomerPoint, User.AdressService.AdressID);
        ...
        private void CalculateOrderPrice()
        {
            ...
            if (counter * 20 <= 150)
            {
                if (counter <= 5)
                {
                    OrderPriceService = new PriceType1_DIP_13();
                    OrderPrice = OrderPriceService.CalculatePrice(counter);
                }
                else
                {
                    OrderPriceService = new PriceType2_DIP_13();
                    OrderPrice = OrderPriceService.CalculatePrice(counter);
                }
            ...
            {
                OrderPriceService = new PriceType3_DIP_13();
                OrderPrice = OrderPriceService.CalculatePrice(counter);
                Console.WriteLine("Number of items in your cart: " + counter);
